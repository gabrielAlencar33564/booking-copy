(function () {
  ...
  window.initReviewCard = function initReviewCard({
    ...
    if (!root) {
      console.warn("[reviewCard] container não encontrado:", rootSelector);
      return;
    }
    const card = root;
    ...
    const score = Number(dataset?.summary?.score ?? 0);
    const label = dataset?.summary?.label ?? "";
    const count = Number(dataset?.summary?.count ?? 0);
    ...
    if (bottomLabelEl)
      bottomLabelEl.textContent = dataset?.summary?.cityNote || "Nota em alta na região";
    const avgCat =
      Array.isArray(dataset?.categories) && dataset.categories.length
        ? Math.min(
            10,
            dataset.categories.reduce((acc, c) => acc + (Number(c.value) || 0), 0) /
    ...
    const reviews = Array.isArray(dataset?.loved?.reviews) ? dataset.loved.reviews : [];
    let index = 0;
